...
<div class="platform-index-form" style='height: 300px; width: 300px;'>
	...
	<div style="width: 100%; background: #ccc">
		<?php echo Html::button('手机快速注册',['id'=>'btn_quick_register','class' => 'btn btn-primary','style'=>'width:100%;height:60px;font-size:20px;'])?>
	</div>
</div>
...
function resizeContainer(){
	$(".platform-index-form").css({'width':$(window).width(),'height':$(window).height()-$('footer').height()});
	$(".container_bg").css({'width':$(window).width(),'height':$(window).height()-$('footer').height()});
}
